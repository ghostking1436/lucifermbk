...
def update_soul_txt(public_url):
    with open("lucifermbk.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in lucifermbk.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("lucifermbk.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated lucifermbk.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update lucifermbk.txt on VPS: {str(e)}")
